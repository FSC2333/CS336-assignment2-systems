...
#         "--detailed-attn-nvtx",
#         action="store_true",
#         help="Add fine-grained NVTX ranges inside self-attention forward for qk matmul, softmax, and av matmul.",
#     )
...
# def language_modeling_loss(model: torch.nn.Module, tokens: torch.Tensor, targets: torch.Tensor, use_nvtx: bool) -> torch.Tensor:
#     with nvtx_range("step/forward", enabled=use_nvtx):
#         logits = model(tokens)
#     with nvtx_range("step/loss", enabled=use_nvtx):
...
#             with nvtx_range("step/forward_backward", enabled=use_nvtx):
#                 with nvtx_range("step/zero_grad", enabled=use_nvtx):
#                     model.zero_grad(set_to_none=True)
#                 loss = language_modeling_loss(model, tokens, targets, use_nvtx)
#                 with nvtx_range("step/backward", enabled=use_nvtx):
#                     loss.backward()
...
#             with nvtx_range("step/training_step", enabled=use_nvtx):
#                 with nvtx_range("step/zero_grad", enabled=use_nvtx):
#                     optimizer.zero_grad(set_to_none=True)
#                 loss = language_modeling_loss(model, tokens, targets, use_nvtx)
#                 with nvtx_range("step/backward", enabled=use_nvtx):
#                     loss.backward()
#                 with nvtx_range("step/optimizer_step", enabled=use_nvtx):
#                     optimizer.step()
...
#     if args.detailed_attn_nvtx:
#         if not args.nvtx:
#             raise ValueError("--detailed-attn-nvtx requires --nvtx")
#         if device.type != "cuda":
#             raise ValueError("--detailed-attn-nvtx requires a CUDA device")
#         os.environ["CS336_DETAILED_ATTN_NVTX"] = "1"
#     else:
#         os.environ.pop("CS336_DETAILED_ATTN_NVTX", None)
...
#         step_fn = make_step_fn(mode, model, optimizer, tokens, targets, use_nvtx=args.nvtx)
#         results.append(
#             benchmark_step(
#                 mode,
#                 step_fn,
