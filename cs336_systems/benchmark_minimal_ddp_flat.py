...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Compare naive per-parameter DDP all-reduce against flat-gradient DDP all-reduce.")
    parser.add_argument("--model-size", choices=sorted(MODEL_CONFIGS), default="xl")
    parser.add_argument("--modes", choices=(*DDP_MODES, "both"), nargs="+", default=["both"])
...
def build_model(args: argparse.Namespace, config: ModelConfig, device: torch.device, mode: str) -> torch.nn.Module:
...
def benchmark_training(
    *,
    ...
    for step in range(args.measurement_steps):
        ...
        timings.append(LocalStepTiming(mode=mode, rank=rank, step=step, total_s=total_s, communication_s=communication_s))

    distributed_barrier(device)
    return timings


def summarize_timings(mode: str, rank_timings: list[list[LocalStepTiming]], measurement_steps: int) -> list[StepTimingSummary]:
    summaries = []
...
def worker(rank: int, args: argparse.Namespace, backend: str, modes: tuple[str, ...], master_port: int, result_queue: mp.SimpleQueue) -> None:
    ...
    try:
        ...
        for mode_idx, mode in enumerate(modes):
            ...
            model = build_model(args, config, device, mode)
...
def run_benchmark(args: argparse.Namespace, backend: str, modes: tuple[str, ...]) -> tuple[int, list[StepTimingSummary]]:
...
def print_results(args: argparse.Namespace, backend: str, modes: tuple[str, ...], num_params: int, summaries: list[StepTimingSummary]) -> None:
    config = MODEL_CONFIGS[args.model_size]
    local_batch_size = args.global_batch_size // args.world_size
    ...
    print("Minimal DDP flat-gradient benchmark setting")
    ...
    print()
    print(f"{'mode':<8} {'iter':>6} {'total max ms':>14} {'comm max ms':>14} {'comm %':>8} {'total mean ms':>14} {'comm mean ms':>14}")
    print("-" * 91)
    ...
    print()
    print(f"{'mode':<8} {'avg total max ms':>18} {'avg comm max ms':>17} {'avg comm %':>12}")
    print("-" * 60)
    for mode in modes:
        mode_rows = [summary for summary in summaries if summary.mode == mode]
        print(
            f"{mode:<8} "
            f"{mean([row.max_total_ms for row in mode_rows]):18.3f} "
            f"{mean([row.max_communication_ms for row in mode_rows]):17.3f} "
            f"{mean([row.max_communication_fraction for row in mode_rows]) * 100:12.2f}"
        )
