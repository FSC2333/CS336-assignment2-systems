...
class RMSNorm(nn.Module): # 均方根归一化，把输入的数据强行按比例缩放到一个稳定的范围内，防止数据在经过神经网络深层传递时变得过大（爆炸）或过小（消失），同时它的计算速度比传统归一化更快。
    """
...
class RotaryEmbedding(nn.Module):
    ...
    @staticmethod
    def _init_cache(context_length: int, dim: int, theta: float) -> Float[Tensor, " 2 context_length half_dim"]: #为什么是2？ 因为被拆分为两两一组
        assert dim % 2 == 0

        d = torch.arange(0, dim, 2) / dim # [0, 2, 4, ..., dim-2] / dim
        freqs = torch.tensor(theta) ** -d
        t = torch.arange(context_length)

        freqs = einsum(t, freqs, "t, f -> t f") # 形状：[context_length, dim // 2]
    ...
    def forward(
        ...
        x1, x2 = rearrange(x, "... (half_d xy) -> xy ... half_d", xy=2).unbind(0) # 分奇偶拆分
...
class BasicsTransformerLM(nn.Module):
    ...
    @torch.no_grad() # 让pytorch 不要记录梯度计算需要的信息
    def generate(
        self,
        x: torch.Tensor,
        max_new_tokens: int,
        ...
        for _ in range(max_new_tokens):
            ...
            next_token_logits = logits[:, -1] # 只取最后一个token的logits  next_token_logits.shape == [B, V]
            # apply temperature scaling
            temperature_scaled_next_token_logits = next_token_logits / temperature
            # temperature小于1时，分布会变得更尖锐，模型更有可能选择概率最高的token；temperature大于1时，分布会变得更平坦，模型更有可能选择概率较低的token。
            # If top-k is provided, take the tokens with the highest score
            if top_k:
                topk_values, _ = torch.topk(
                    temperature_scaled_next_token_logits,
                    min(top_k, temperature_scaled_next_token_logits.size(-1)), # topk_values.shape == [B, top_k]
                )
                # Get the score of the kth item that we kept---items with lower scores should be masked.
                threshold = topk_values[:, -1] # 从大到小排序，取最后一个元素作为阈值 threshold.shape   == [B]
                topk_mask = temperature_scaled_next_token_logits < threshold # 通过最小的topk的阈值进行掩码计算
            ...
            next_token_id = torch.multinomial(next_token_probabilities, 1) # 随机采样
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_path: str):  # 从配置文件加载模型参数
...
#   ↓
# 残差连接：attention_result + ffn_output
#   ↓
# 输出
# #####################
...
class CausalMultiHeadSelfAttention(nn.Module):
    ...
    def forward(
        ...
        *batch_dims, sequence_length, d_model = x.size()
        ...
        iota = torch.arange(sequence_length, device=x.device) # iota = [0, 1, 2, ..., sequence_length - 1]
        qi = rearrange(iota, "query -> query 1")
        kj = rearrange(iota, "key   -> 1   key")
        causal_mask = qi >= kj  # (query, key) 下三角为true的矩阵
